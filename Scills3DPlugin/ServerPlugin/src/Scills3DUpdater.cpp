...
//     assert(0);
    ...
    qDebug() << type_parametric[ "type" ];
    PRINT(name_parametric_scheme);
    
    
    if (name_parametric_scheme == "off") { data_user.multiresolution_parameters.multiresolution_type = "off";}
    else if (name_parametric_scheme == "function") { data_user.multiresolution_parameters.multiresolution_type = "function";}
    
    data_user.multiresolution_parameters.resolution_number = convert_MP_to_int(parametric_parameters[ "nb_resolutions" ]);
    int nb_parametric_variables = convert_MP_to_int(parametric_parameters[ "nb_paramaters" ]);
    ...
    for(int i_param = 0;i_param < nb_parametric_variables; ++i_param){
        MP  parametric_function_i = parametric_parameters[ "_children" ][ i_param ];
        data_user.multiresolution_parameters.collection_vec[i_param].id_in_calcul               = convert_MP_to_int(parametric_function_i[ "_id" ]);
        data_user.multiresolution_parameters.collection_vec[i_param].parametric_function        = convert_MP_to_Sc2String(parametric_function_i[ "your_function" ]);
        data_user.multiresolution_parameters.collection_vec[i_param].alias_name                 = convert_MP_to_Sc2String(parametric_function_i[ "_name" ]);
        data_user.multiresolution_parameters.collection_vec[i_param].name                       = convert_MP_to_Sc2String(parametric_function_i[ "_name" ]);
        data_user.multiresolution_parameters.collection_vec[i_param].max_value                  = convert_MP_to_reel(parametric_function_i[ "bound[1]" ]);
        data_user.multiresolution_parameters.collection_vec[i_param].min_value                  = convert_MP_to_reel(parametric_function_i[ "bound[0]" ]);
        data_user.multiresolution_parameters.collection_vec[i_param].nb_value                   = convert_MP_to_int(parametric_parameters[ "nb_resolutions" ]);
    }
    ...
    for(int i_param = 0;i_param < nb_parametric_variables; ++i_param){
        ...
        PRINT(data_user.multiresolution_parameters.collection_vec[i_param].parametric_function);
        PRINT(data_user.multiresolution_parameters.collection_vec[i_param].alias_name);
        PRINT(data_user.multiresolution_parameters.collection_vec[i_param].name);
        PRINT(data_user.multiresolution_parameters.collection_vec[i_param].max_value);
        PRINT(data_user.multiresolution_parameters.collection_vec[i_param].nb_value); 
    }  
...
//         process.finalisation_MPI();
    ...
    mp.flush();
    add_message( mp, ET_Info, "Scills3DUpdater just finish" );
}
